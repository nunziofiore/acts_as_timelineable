      ...
      module ClassMethods
        def acts_as_timelineable(*args)
          ...
          if !timeline_roles.blank?
            ...
            has_many :all_timelines, {:as => :timelined, :dependent => :destroy, class_name: "Timeline"}
          else
          ...
          timeline_types.each do |role|
            ...
            class_eval %{
              def self.find_#{method_name}_for(obj)
                timelined = self.base_class.name
                Timeline.find_timelines_for_timelined(timelined, obj.id, "#{role.to_s}")
              end

              def self.find_#{method_name}_by_user(user) 
                timelined = self.base_class.name
                Timeline.where(["user_id = ? and timelined_type = ? and role = ?", user.id, timelined, "#{role.to_s}"]).order("created_at DESC")
              end

              def #{method_name}_ordered_by_submitted
                Timeline.find_timelines_for_timelined(self.class.name, id, "#{role.to_s}").order("created_at")
              end
...
ActiveRecord::Base.send(:include, Laboop::Acts::Timelined)
